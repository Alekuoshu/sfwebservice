...
class OnePageCheckoutPSOverride extends OnePageCheckoutPS
{
    const VERSION = '1.1.7';
    ...
    public function initContentControllerOPC($controller)
    {
        if ($this->config_vars['OPC_REDIRECT_DIRECTLY_TO_OPC']
            ...
            $templateVars = $this->getTemplateVarsOPC($this->only_register, $show_authentication);
    ...
    protected function displayForm()
    {
        ...
        if ($registered_version != $this->version) {
            ...
            $this->smarty->assign(array(
                ...
                'message_updated' => $this->l('We have detected you uploaded the new version'),
                'message_module' => $this->l('of our module'),
                'message_click' => $this->l('To proceed with the update, you need to click here'),
                'message_now' => $this->l('Update now')
            ));

            $this->html = $this->display(__FILE__, 'views/templates/admin/helper/update_version.tpl');
...
//			$default_value = Tools::substr($default_value, 0, $field_class->size);
                    ...
                    foreach ($fields as $position => $field) {
                        ...
                        if ($field->name == 'passwd') {
                            ...
                            if (isset($opc_fields[$field->object.'_conf_email'])) {
                                ...
                                $opc_fields_position[$group][$row + 1][$position] = $opc_fields[$index];
                            }
                        }
                    }
                }
    ...
    public function getMediaFront()
    {
        ...
        if (!$this->config_vars['OPC_REDIRECT_DIRECTLY_TO_OPC'] || !Tools::getIsset('rc') || Tools::getIsset('checkout')) {
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        $query->innerJoin('opc_field_lang', 'fl', 'fl.id_field = fc.id_field AND fl.id_lang = '.(int)$this->cookie->id_lang);
        $query->leftJoin(
            'opc_field_option_lang',
            'fol',
            'fc.id_option = fol.id_field_option AND fol.id_lang = '.(int)$this->cookie->id_lang
        );
        $query->where('fc.id_cart = '.(int)$order->id_cart);
    ...
    public function hookActionDeleteGDPRCustomer($customer)
    {
        if (Validate::isLoadedObject($customer)) {
            $sql = "DELETE FROM "._DB_PREFIX_."opc_social_customer WHERE id_customer = '".(int)$customer->id."'";
    ...
    private function validateFieldsAddress(&$fields, &$address)
    {
        foreach ($fields as $name => $field) {
            ...
            } elseif (!empty($field->value) && !$valid) { //aqui
...
//					}
        ...
        /* compatibility vatnumbercleaner v.1.2.8 by MassonVincent */
        $vatnumbercleaner = $this->core->isModuleActive('vatnumbercleaner');

        if ($vatnumbercleaner && !empty($address->vat_number)) {
            $id_country = $address->id_country;
            $vat_number = $address->vat_number;

            $verifications = $vatnumbercleaner->verificationsIdAddress((int)$address->id, $vat_number, $id_country);

            if ((int)$verifications['id_msg'] > 0 && !defined('_PS_ADMIN_DIR_')) {
                $this->errors[] = $vatnumbercleaner->msgerror($verifications['id_msg']);
            }
        }
    ...
    public function loadAddress()
    {
        ...
        $sql = new DbQuery();
        $sql->select('f.name, fc.value');
        $sql->from('opc_field_cart', 'fc');
        $sql->innerJoin('opc_field', 'f', 'f.id_field = fc.id_field');
        $sql->where('fc.id_cart = '.(int) $this->context->cart->id);

        $custom_fields = Db::getInstance()->executeS($sql);
        ...
        if ($this->context->customer->isLogged()) {
            ...
            if (Validate::isLoadedObject($address)) {
                if ($this->context->customer->id == $address->id_customer) {
                    ...
                    $query = 'DELETE FROM '._DB_PREFIX_.'opc_customer_address WHERE id_address = '.(int)$id_address;
                    Db::getInstance()->execute($query);
    ...
    public function updateAddress()
    {
        ...
        // capturando el tipo de documento
        $document_type = Tools::getValue('type_dni');
        ...
        $this->validateFields($fields, $customer, $address, $address, $password, $is_set_invoice);
        ...
        if (Validate::isLoadedObject($address)) {
            ...
            $address->type_dni = $document_type;
            $address->update();
    ...
    public function addModulesExtraFee()
    {
        ...
        //comaptibilidad con modulo paypalwithfee - v4.0.10 - 4webs.es
    ...
    public function deleteEmptyAddressesOPC()
    {
        ...
        if (count($carts) > 0) {
            foreach ($carts as $cart) {
                ...
                if ((int)$cart['id_customer'] == (int)$this->config_vars['OPC_ID_CUSTOMER']) {
    ...
    public function getTemplateVarsOPC($only_register, $show_authentication)
    {
        ...
        $messageValidate = array(
            ...
            'errorIsPasswd'         => $this->l('La Contraseña debe ser mínimo de 8 caracteres. Usa una combinación letras y números.'),
        ...
        $templateVars = array(
            ...
            'is_virtual_cart'               => (bool)$this->context->cart->isVirtualCart(),
            ...
            'OnePageCheckoutPS' => array(
                ...
                'Msg' => array(
                    ...
                    'info_edit_same_address' => $this->l('You will edit an address that is being used as a shipping and billing address, when changing the information will change in both places, if you want a new address, close and click on the "Add new address" button.'),
        ...
        if ($this->core->isModuleActive('vatnumbercleaner')) {
            $templateVars['vatnumbercleaner'] = true;
        }
    ...
    public function validatePassw()
    {
        // *** Passw validation...***
        $passwValue = Tools::getValue('password');
            ...
            if (!empty($passwValue)) {
                ...
                if(!$regexValid){
                    $this->errors[] = $this->l('La Contraseña debe ser mínimo de 8 caracteres. Usa una combinación letras y números.');
                }
            }
